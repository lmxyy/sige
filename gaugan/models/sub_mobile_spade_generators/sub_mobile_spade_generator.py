...
class SubMobileSPADEGenerator(nn.Module):
    def __init__(self, opt, config, **kwargs):
        ...
        self.head_0 = SubMobileSPADEResnetBlock(
            16 * nf, 16 * nf, ic, opt, {"channel": channel * 16, "hidden": channel * 2}
        )

        channel = config["channels"][2]
        self.G_middle_0 = SubMobileSPADEResnetBlock(
            16 * nf, 16 * nf, ic, opt, {"channel": channel * 16, "hidden": channel * 2}
        )

        channel = config["channels"][3]
        self.G_middle_1 = SubMobileSPADEResnetBlock(
            16 * nf, 16 * nf, ic, opt, {"channel": channel * 16, "hidden": channel * 2}
        )
