...
from ..sige_normalization import SIGEFusedSubMobileSPADE


class SIGEFusedSubMobileSPADEResnetBlock(SIGEModule):
    def __init__(self, fin, fout, ic, opt, config, support_sparse: bool = False):
        super(SIGEFusedSubMobileSPADEResnetBlock, self).__init__()
        # Attributes
        self.fin = fin
        self.fout = fout
        self.ic = ic
        ...
        self.mlp_shared = nn.Sequential(
            MainConv2d(opt.semantic_nc, hidden * (3 if self.learned_shortcut else 2), kernel_size=3, padding=1),
            nn.ReLU(),
        )
        self.conv_0 = MainConv2d(ic, channel, kernel_size=3, padding=1)
        if self.learned_shortcut:
            self.conv_1 = MainConv2d(channel, channel, kernel_size=3, padding=1)
        else:
            self.conv_1 = MainConv2d(channel, ic, kernel_size=3, padding=1)
        ...
        if self.learned_shortcut:
            ...
            self.conv_s = ShortcutConv2d(ic, channel, kernel_size=1, bias=False)
        ...
        self.norm_0 = SIGEFusedSubMobileSPADE(
            spade_config_str,
            fin,
            nhidden=hidden,
            oc=ic,
        ...
        self.norm_1 = SIGEFusedSubMobileSPADE(
            spade_config_str,
            fmiddle,
            nhidden=hidden,
            oc=channel,
        ...
        if self.learned_shortcut:
            self.norm_s = SIGEFusedSubMobileSPADE(
                spade_config_str,
                fin,
                nhidden=hidden,
                oc=ic,
...
class SIGEFusedSubMobileSPADEGenerator(SIGEModel):
    def __init__(self, opt, config, **kwargs):
        super(SIGEFusedSubMobileSPADEGenerator, self).__init__()
        self.opt = opt
        self.config = config
        nf = opt.ngf
        ...
        self.head_0 = SIGEFusedSubMobileSPADEResnetBlock(
            16 * nf,
            16 * nf,
            ic,
            opt,
            {"channel": channel * 16, "hidden": channel * 2},
            support_sparse=num_sparse_layers >= 7 + is_most,
        )

        channel = config["channels"][2]
        self.G_middle_0 = SIGEFusedSubMobileSPADEResnetBlock(
            16 * nf,
            16 * nf,
            ic,
            opt,
            {"channel": channel * 16, "hidden": channel * 2},
            support_sparse=num_sparse_layers >= 6 + is_most,
        )

        channel = config["channels"][3]
        self.G_middle_1 = SIGEFusedSubMobileSPADEResnetBlock(
            16 * nf,
            16 * nf,
            ic,
            opt,
            {"channel": channel * 16, "hidden": channel * 2},
            support_sparse=num_sparse_layers >= 5 + is_most,
        )

        channel = config["channels"][4]
        self.up_0 = SIGEFusedSubMobileSPADEResnetBlock(
            16 * nf,
            8 * nf,
            ic,
            opt,
            {"channel": channel * 8, "hidden": channel * 2},
            support_sparse=num_sparse_layers >= 4 + is_most,
        )

        ic = channel * 8
        channel = config["channels"][5]
        self.up_1 = SIGEFusedSubMobileSPADEResnetBlock(
            8 * nf,
            4 * nf,
            ic,
            opt,
            {"channel": channel * 4, "hidden": channel * 2},
            support_sparse=num_sparse_layers >= 3 + is_most,
        )
        ic = channel * 4
        channel = config["channels"][6]
        self.up_2 = SIGEFusedSubMobileSPADEResnetBlock(
            4 * nf,
            2 * nf,
            ic,
            opt,
            {"channel": channel * 2, "hidden": channel * 2},
            support_sparse=num_sparse_layers >= 2 + is_most,
        )
        ic = channel * 2
        channel = config["channels"][7]
        self.up_3 = SIGEFusedSubMobileSPADEResnetBlock(
            2 * nf,
            1 * nf,
            ic,
            opt,
            {"channel": channel, "hidden": channel * 2},
            support_sparse=num_sparse_layers >= 1 + is_most,
        )
