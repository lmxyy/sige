...
class DRN(nn.Module):
    def __init__(
        ...
        self.layer6 = (
            None if layers[5] == 0 else self._make_layer(block, channels[5], layers[5], dilation=4, new_level=False)
        )
        ...
        if arch == "C":
            self.layer7 = (
                None
                if layers[6] == 0
                else self._make_layer(BasicBlock, channels[6], layers[6], dilation=2, new_level=False, residual=False)
            )
            self.layer8 = (
                None
                if layers[7] == 0
                else self._make_layer(BasicBlock, channels[7], layers[7], dilation=1, new_level=False, residual=False)
            )
        elif arch == "D":
