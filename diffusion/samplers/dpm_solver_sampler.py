...
class DPMSolverSampler(BaseSampler):
    ...
    def denoising_steps(self, x: torch.Tensor, model: nn.Module, seq, **kwargs):
        ...
        seq = [-1] + list(seq)
        sampling_tqdm = mytqdm(reversed(seq), total=len(seq), desc="Sampling   ", position=tqdm_position, leave=False)
        ...
        with torch.no_grad():
            for step, t in enumerate(sampling_tqdm):
                ...
                if step == 0:
                    ...
                    model_prev_list = [self.model_step(model, x, t)]
                elif step < order:
                    ...
                    model_prev_list.append(self.model_step(model, x, t))
                else:
    ...
    def model_step(self, model, xt, t):
